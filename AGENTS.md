...
Project-wide instructions are duplicated in sibling ArborXR repos (abxrlib-for-unity, abxrlib-for-unreal); keep shared guidelines in sync when updating.
...
- **Init:** Default path uses **app tokens**: `Abxr_init({ appToken, orgToken?, buildType? })`. Legacy path: `Abxr_init(appId, orgId?, authSecret?, ...)`. For `buildType === "production_custom"`, `orgToken` is required.
